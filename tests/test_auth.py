    ...
    @pytest.mark.parametrize(
        ('firstname', 'lastname', 'username', 'email', 'password', 'code', 'func'), [
            pytest.param('Mike', 'Henry', 'MikeH123', 'MikeHenry@gmail.com',
                         'T3$tData1234', 200, lambda resp: has_refresh_token(resp), id='UT1'),
            pytest.param('Mike', 'Henry', 'MikeH123', 'MikeHenry@gmail.com',
                         'T3$tData12', 400, lambda resp: has_pwd_length_error(resp), id='UT2'),
            pytest.param('Mike', 'Henry', 'MikeH123', 'MikeHenry@gmail.com', 'TestData123', 400,
                         lambda resp: has_pwd_length_error(resp) or has_pwd_form_error(resp), id='UT3'),
            pytest.param('Mike123', 'Henry', 'MikeH123', 'MikeHenry@gmail.com', 'T3$tData1234',
                         400, lambda resp: has_invalid_firstname_error(resp), id='UT4'),
            pytest.param('Mike', 'Henry#', 'MikeH123', 'MikeHenry@gmail.com', 'T3$tData1234',
                         400, lambda resp: has_invalid_lastname_error(resp),  id='UT5'),
            pytest.param('Mike', 'Henry', 'MikeH123', 'MikeHenrygmail.com', 'T3$tData1234',
                         400, lambda resp: has_invalid_email_error(resp), id='UT6'),
            pytest.param('Mike', 'Henry', 'M!keH123', 'MikeHenry@gmail.com', 'T3$tData1234',
                         400, lambda resp: has_username_form_error(resp), id='UT7'),
            pytest.param('Mike', 'Henry', 'Mike', 'MikeHenry@gmail.com', 'T3$tData1234',
                         400, lambda resp: has_username_length_error(resp), id='UT8'),
            pytest.param('Mike', 'Henry', 'MikeHenry_Test1234567890DataTooLong', 'MikeHenry@gmail.com',
                         'T3$tData1234', 400, lambda resp: has_username_length_error(resp), id='UT9'),
            pytest.param('Mike', 'Henry', 'marckennedy', 'MikeHenry@gmail.com',
                         'T3$tData1234', 409, lambda resp: has_used_username_error(resp), id='UT10'),
            pytest.param('Mike', 'Henry', 'MikeH123', 'daviesoscar@example.org',
                         'T3$tData1234', 409, lambda resp: has_used_email_error(resp), id='UT11')
        ]
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param('daviesoscar@example.org', '#o35Pu@P7KMx',
                         200, lambda resp: has_refresh_token(resp), id='UT12'),
            pytest.param('marckennedy', 'ZBly62WopR$2', 200,
                         lambda resp: has_refresh_token(resp), id='UT13'),
            pytest.param('ThisaccountdoesnotExist', 'T3$tData1234', 401,
                         lambda resp: has_incorrect_user_cred_error(resp), id='UT14'),
            pytest.param('kgarcia', 'T3$tData1234', 401,
                         lambda resp: has_incorrect_user_cred_error(resp), id='UT15'),
            pytest.param('marckennedy', 'T3$tData12', 400,
                         lambda resp: has_pwd_length_error(resp), id='UT16'),
            pytest.param('marckennedy', 'Testdata1234', 400,
                         lambda resp: has_pwd_form_error(resp), id='UT17'),
            pytest.param('dawn21', '@(FJw64bLn5Z', 401,
                         lambda resp: has_user_deleted_error(resp), id='UT18')
        ]
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param('marckennedy', 'ZBly62WopR$2', False, None,
                         200, lambda resp: has_logout_success(resp), id='UT19'),
            pytest.param('marckennedy', 'ZBly62WopR$2', True, None, 200,
                         lambda resp: has_invalid_token_error(resp), id='UT20'),
            pytest.param(None, None, False, None, 401,
                         lambda resp: has_invalid_token_error(resp), id='UT21'),
            pytest.param(None, None, False, 'Hi', 401,
                         lambda resp: has_invalid_token_error(resp), id='UT22')
        ]
    )
